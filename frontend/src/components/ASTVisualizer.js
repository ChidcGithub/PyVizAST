...
import { getLLMConfig, getLLMStatus } from '../api';
...
const ASTVisualizer = forwardRef(function ASTVisualizer({ graph, theme, onGoToLine, gestureEnabled = false }, ref) {
  ...
  // LLM config (for determining panel visibility and styling)
  ...
  useEffect(() => {
    const fetchLLMConfig = async () => {
      try {
        ...
        logger.debug('LLM config fetched', { config, status });
        setLLMConfig({
          ...config,
          status: status.status
        });
      } catch (err) {
        logger.debug('Could not fetch LLM config', { error: err.message });
        setLLMConfig(null);
      }
    };
    ...
    // Listen for LLM config changes from LLMSettings
    const handleLLMConfigChange = (event) => {
      logger.debug('LLM config changed event received', event.detail);
      const { config, status } = event.detail || {};
      if (config) {
        // Use the actual status from the response, not a default
        const actualStatus = status?.status;
        logger.debug('Setting LLM config with status', { config, actualStatus });
        setLLMConfig({
          ...config,
          status: actualStatus || 'unavailable'
        });
      } else {
        // Refetch if detail not provided
        fetchLLMConfig();
      }
    };
    
    window.addEventListener('llmConfigChanged', handleLLMConfigChange);
    
    return () => {
      window.removeEventListener('llmConfigChanged', handleLLMConfigChange);
    };
  }, []);
  ...
  const runCursorAnimation = useCallback(() => {
    ...
    // Keep running if: visible OR still interpolating position (not at target yet)
    const isMoving = Math.abs(state.x - state.targetX) > 0.5 || Math.abs(state.y - state.targetY) > 0.5;
    
    if (!state.visible && !isMoving) {
    ...
    if (state.hoveredNodeId && state.hoverStartTime > 0) {
      ...
      if (state.progress >= 100 && cyRef.current) {
        ...
        // Reset hover state but don't stop animation - let it continue for smooth cursor movement
        state.hoveredNodeId = null;
        state.progress = 0;
        state.hoverStartTime = 0;
        // Note: We don't set visible=false here, allowing the cursor to continue following
        // The cursor will be hidden when the user stops pointing (via clearPointingCursor)
      }
    }
  ...
  return (
    <div className="ast-visualizer">
      ...
      <div className="visualizer-content">
        ...
        {selectedNode && (
          <div className={`node-detail-panel${llmConfig?.enabled && llmConfig?.use_for_explanations ? ' has-llm-explanation' : ''}`}>
            ...
            <div className="panel-body">
              ...
              {/* LLM AI Explanation - Unified Component */}
              <LLMExplanationPanel
                llmConfig={llmConfig}
